...
private:
	...
	///////////////////////////////////////////////////
	// <your declarations>
	int missTable[1000000], hitTable[1000000];
	// </your declarations>
	///////////////////////////////////////////////////
};
	...
	laserScanFilter(laserScanSubscriber, tfListener, "odom", 5)
{
	...
	// Initialize miss table and hit table using the same code as 
	// above
	for (unsigned int i = 0; i < this->map.info.width; i++) {
		for (unsigned int j = 0; j < this->map.info.height; j++) {
			int index = j*this->map.info.height + i;
			missTable[index] = 0;
			hitTable[index] = 0; // -1 means unknown
...
void MappingNode::laserReceived( const sensor_msgs::LaserScanConstPtr& laserScan ) {
	...
	//process laserscan output by calculating all the points
	int scanCount = sizeof(laserScan->ranges);

	float angleMin = laserScan->angle_min;

	float angleMax = laserScan->angle_max;
	...
	//For each scan value in the ranges array
	// for (scanVar = 0; scanVar < scanCount; scanVar++) {
	while (currentAngle < angleMax) {
		currentAngle = angleMin + (scanVar * angleIncrement);
		omega = robotTheta + currentAngle;

		if (laserScan->ranges[scanVar] > rangeMin && laserScan->ranges[scanVar] 
		< rangeMax) {
			
			//Convert to metre space
			currentRange = laserScan->ranges[scanVar] / this->map.info.resolution;

			p = currentRange * cos(omega) + robotX;

			q = currentRange * sin(omega) + robotY;

			pair<int,int> hitPoint;

			result = bresenhamLine(robotX, robotY, p, q);

			//Process the last point as hit before discarding the value
			hitPoint = result.back();
			int hitX = hitPoint.first;
			int hitY = hitPoint.second;

			int hitIndex = hitY*this->map.info.width + hitX;

			hitTable[hitIndex] = hitTable[hitIndex] + 1;

			//Process the hit point
			value = hitTable[hitIndex] / (hitTable[hitIndex] + missTable[hitIndex]);
			updateReflectionMapValue(hitX, hitY, value);

			result.pop_back();

			//Process all the miss points
			for (vector <pair <int,int> >::iterator point = result.begin(); point != result.end(); point++ ) {
				
				int index = point->second*this->map.info.width + point->first;
				missTable[index] += 1; 
			
				value = hitTable[index] / (hitTable[index] + missTable[index]);
				updateReflectionMapValue(point->first, point->second, value);
			}
